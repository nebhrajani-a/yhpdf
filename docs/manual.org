...
** Basic
   ...
   1. Install *Ghostscript (64-bit)*:
      ...
      B. Download the file *Ghostscript 9.54.0 for Windows (64 bit)* with
         the AGPL license (first column). _Do not use the 32 bit
   ...
   2. Get *yhpdf*:
      ...
      C. Double click on =yhpdf=. _Wait for 10 seconds_. If it doesn't
      ...
      D. Once it starts, you should see a windows with four buttons and
         some text. Go down to the taskbar, right click on =yhpdf='s
         feather icon, and select "Pin to taskbar".
      E. If =yhpdf= doesn't start and reports an error, make sure
...
* Usage
  ...
  3. Click "Process PDF" to process the PDF. This can take up to 60
     ...
     the input PDF.
     1. Once the PDF process is complete, a popup will ask you if you
